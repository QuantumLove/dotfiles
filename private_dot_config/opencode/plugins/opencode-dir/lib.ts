// ---------------------------------------------------------------------------
// VENDORED COPY of opencode-dir's lib.ts — see VENDORED.md in this directory.
//
// Source: https://github.com/adiled/opencode-dir (MIT) @ v1.0.10
// Local changes vs upstream: ALL phone-home telemetry removed — the Sentry
// error reporter and the npm self-update check. Nothing here makes network
// calls. To upgrade: re-vendor from upstream and re-apply these removals.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Error reporting — telemetry REMOVED in this vendored copy.
//
// Upstream sent every caught error to a hardcoded Sentry DSN over the network.
// That has been stripped so the plugin never phones home. This is now a no-op,
// kept only so the existing call sites keep compiling; errors still surface to
// the user via opencode toasts and the catch blocks below.
// ---------------------------------------------------------------------------

export function reportError(_err: Error): void {
  // intentionally empty — no network, no telemetry
}
...
// checkForUpdate() and its UpdateResult type were REMOVED in this vendored copy.
// Upstream fetched registry.npmjs.org and deleted files in opencode's plugin
// cache to force a re-install. We pin/upgrade manually instead — see VENDORED.md.
// index.ts no longer imports or calls it.
