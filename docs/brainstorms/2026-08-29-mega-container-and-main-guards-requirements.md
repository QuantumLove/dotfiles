...
- R7. Branch policy has exactly one definition, and git hooks are the enforcement layer.
- R8. Repo identity is determined by git remote slug everywhere, never by filesystem path.
- R9. Authorization is evaluated against the repo the git operation targets, not the ambient
  session directory.
- R10. Enforcement does not depend on matching command surface forms.
- R11. The exception list covers both `QuantumLove/dotfiles` and `QuantumLove/glove80`, and
  applies uniformly to edit, checkout, commit, and push.
- R12. Commit on a protected branch is mechanically enforced rather than left to prose.
- R13. The guard remains active when HEAD is detached.
- R14. The protected branch is derived from the repo's actual default branch rather than a
  hardcoded `main`/`master` list.
- R15. Enforcement applies to agents running inside the container, not only Claude Code on the
  host.
- R16. `--no-verify` is the only bypass, and no agent configuration, skill, or command template
  distributes it.
- R17. `glove80` is checked out locally so its guard path is exercised.

**Container boot**

- R18. Secrets are classified as required or optional, and a missing optional secret does not
  block boot.
- R19. Plugin installation has a single implementation.

**Agent harness**

- R20. `omp` is installed, configured, and usable in the container alongside `omo`.
- R21. Forks of `oh-my-pi` and `oh-my-opencode` exist under our control, pinned to our own
  releases.
- R22. Model selection for every agent in every harness is declared in committed configuration.
- R23. Declared model selection and fork version resolution are both covered by assertions.

**Terminal and remote access**

- R24. Pane splits subdivide the focused pane by default, with full-span splits bound
  separately.
- R25. tmux bindings and the Glove80 `tmux` layer change together, because a layer key emitting
  an unbound symbol fails silently.

- R26. A single helper exposes a container port to the tailnet, covering both loopback-bound
  services and those already on `0.0.0.0`, and reports the reachable URL.

- R27. Copying the current path yields the agent's working directory while an agent is running,
  and the shell's directory otherwise.
- R28. The `oc` wrapper's session-listing command works, rather than passing an invocation that
  upstream never defined.
- R29. A session view lists sessions across all projects, grouped by project.
- R30. `omp` has a working-directory redirect equivalent to OpenCode's.

**Process**

- R31. Queue items are independently shippable and individually approved before work starts.
...
- AE2. **Covers R6.** Given a tool rewrites an applied config file, when the suite runs, then
  the divergence from chezmoi source is reported.
- AE3. **Covers R9.** Given the ambient session directory is the chezmoi checkout, when a push
  to a protected branch is issued against a different repository, then it is denied.
- AE4. **Covers R8, R11.** Given the `glove80` repo checked out at any path, when a push to its
  default branch is issued, then it is allowed.
- AE5. **Covers R8.** Given the chezmoi repo re-cloned to a path other than
  `~/.local/share/chezmoi`, when a push to `main` is issued, then it is still allowed.
- AE6. **Covers R10.** Given a push expressed as `git push`, `git push origin HEAD`, or
  `git -C <path> push`, when the target is a protected branch in a non-excepted repo, then each
  form is denied.
- AE7. **Covers R13.** Given the primary worktree has a detached HEAD, when a branch operation
  is issued, then the guard evaluates it rather than standing down.
- AE8. **Covers R14.** Given a repo whose default branch is neither `main` nor `master`, when a
  commit is issued on that branch, then the guard applies.
- AE9. **Covers R15.** Given an agent session running inside the container, when it attempts a
  commit or push to a protected branch, then the same policy applies as on the host.
- AE10. **Covers R16.** Given any agent configuration, skill, or command template, when the
  suite runs, then any occurrence of `--no-verify` is reported.
- AE11. **Covers R24.** Given a window already split into two panes, when a split is issued
  from the focused pane, then the new pane subdivides that pane rather than spanning the window.
- AE12. **Covers R25.** Given the Glove80 `tmux` layer, when any key on it is pressed, then the
  symbol it emits is bound in `dot_tmux.conf`.
- AE13. **Covers R26.** Given a service bound only to loopback inside the container, when the
  helper is invoked for its port, then a URL reachable from the Mac is returned.
- AE14. **Covers R27.** Given an agent that has moved to a subdirectory, when the path-copy
  binding is pressed, then the agent's directory is copied rather than the shell's.
- AE15. **Covers R27.** Given no agent is running in a pane, when the path-copy binding is
  pressed, then the shell's directory is copied.
- AE16. **Covers R29.** Given sessions created in several different project directories, when
  the session view is opened from any directory, then all of them are listed.
- AE17. **Covers R18.** Given one optional secret is absent from 1Password, when the container
...
- The Glove80 `tmux` layer currently emits `LS(BSLH)` and `LS(MINUS)`; changing split behaviour
  requires edits in the `QuantumLove/glove80` repo and a keymap regeneration, so R17 (clone
  glove80 locally) is a prerequisite for R24 rather than an independent item.
